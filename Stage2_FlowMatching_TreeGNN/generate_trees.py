"""
Generate vessel trees using the trained Tree-GNN Flow Matching model.

Same pipeline as the Transformer variant but loads TreeFlowNet.

Usage:
    python Stage2_FlowMatching_TreeGNN/generate_trees.py \
        --config Stage2_FlowMatching_TreeGNN/configs/generate_treegnn.yaml
"""
...
def visualize_trees(gen_list, gt_list=None, names=None, output_dir=None):
    ...
    for i, gen_data in enumerate(gen_list):
        ...
        if has_gt and i < len(gt_list):
            ...
            ax2.set_title("Generated (TreeGNN FM)")
            ...
            ax.set_title("Generated (TreeGNN FM)")
...
# ── Postprocessing (same Fixes 1–4 as Transformer variant) ──────────────────
...
def run_pipeline(cfg):
    ...
    output_dir = Path(params.get("output_dir", "Stage2_FlowMatching_TreeGNN/generated/default"))
    ...
    print(f"Loading TreeFlowNet from {ckpt_path}")
