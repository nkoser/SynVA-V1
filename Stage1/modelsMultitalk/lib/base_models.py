...
class MLP(nn.Module):
  ...
  def forward(self, x_data):
    if type(x_data) is dict:
        out = self.l2(self.dropout(self.activation(self.l1(x_data['x_b']))))
        return {'x_a':x_data['x_a'], 'x_b':out}
    else:
        x, mask_info = x_data
        out = self.l2(self.dropout(self.activation(self.l1(x))))
...
class Attention(nn.Module):
  ...
  def __init__(self, in_dim, dim, heads=8, dropout=0.1):
    super().__init__()
    self.heads = heads
    self.scale = dim**-0.5
    ...
    if cross_modal:
      for i in range(num_hidden_layers):
        blocks.extend([
            ...
            Residual(Norm(MLP(hidden_size, hidden_size, intermediate_size, dropout=dropout),
                              hidden_size))
        ])
    else:
      for i in range(num_hidden_layers):
        blocks.extend([
            Residual(Norm(Attention(in_size, hidden_size,
                                    heads=num_attention_heads, dropout=dropout), hidden_size)),
            Residual(Norm(MLP(hidden_size, hidden_size, intermediate_size, dropout=dropout),
                              hidden_size))
        ])
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        # x is expected as [B, L, C]; apply positional encoding over L.
        seq_len = x.size(1)
