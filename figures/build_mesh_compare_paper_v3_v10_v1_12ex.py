"""Paper figure: GT + v3, v10, v1 across 12 cleanest examples.

Layout: 4 rows (GT, v3, v10, v1), 12 cols (examples = 12 cleanest by
avg chamfer over v3/v10/v1 on clean-60).
...
ROWS = [
    ("GT", None),
    ("SynVA-V1$_\\mathrm{depth}$",   "treegnn_v3_healthy_decap_dense_v2_short"),
    ("SynVA-V1$_\\mathrm{aligned}$", "treegnn_v10_focalpos_aligned_short"),
    ("SynVA-V1$_\\mathrm{base}$",    "treegnn_v1_healthy_decap_dense_v2_short"),
]
...
def short_label(uid):
    ...
    for p in ("aneux_", "cmha_", "intra_"):
        if s.startswith(p):
            s = s[len(p):]
            break
    if "_" in s and len(s) > 16:
        s = s.split("_")[0] + "…"
    return s


# ---- pick 12 cleanest by avg chamfer over v1/v3/v10 ----
...
print(f"Picked {len(EXAMPLES)} cleanest trees (avg chamfer over v1/v3/v10):")
...
def build(wireframe):
    ...
    for i, (label, _) in enumerate(ROWS):
        axes[i, 0].set_ylabel(label, fontsize=16, rotation=90, labelpad=10)
        for j in range(ncols):
            draw_panel(axes[i, j], panels.get((i, j)), wireframe=wireframe)

    plt.tight_layout(pad=0.2, w_pad=0.15, h_pad=0.15)
    suffix = "_wf" if wireframe else ""
    stem = f"mesh_compare_paper_v3_v10_v1_12ex{suffix}"
